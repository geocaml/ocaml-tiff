...
module Endian = struct
  ...
  let uint64 ?(offset=0) endian buf = match endian with
...
let header ro =
  ...
  let kind, offset =
    match magic with 
      | 42 -> Tiff, Endian.uint32 ~offset:4 byte_order buf |> Optint.Int63.of_int32 
      | 43 -> Bigtiff, Endian.uint64 ~offset:8 byte_order buf |> Optint.Int63.of_int64
...
module Ifd = struct
  type field = Byte | Ascii | Short | Long | Rational | Sbyte | Undefined
             | Sshort | Slong | Srational | Float | Double | Long8 | Slong8 | Ifd8
  ...
  let rec field_byte_size (x : field) = match x with
    | Byte -> 1
    | Ascii -> 1
    | Short -> 2
    | Long -> 4
  ...
  let v ~file_offset header reader =
    ...
    for i = 0 to count - 1 do
      ...
      | Tiff ->
        ...
        let count = Endian.uint32 ~offset:(i * 12 + 4) endian buf |> Int64.of_int32 in
        let offset = Endian.uint32 ~offset:(i * 12 + 8) endian buf |> Int64.of_int32 in
        entries := { tag; field; count; offset } :: !entries
      | Bigtiff ->
        let tag = Endian.uint16 ~offset:(i * 20) endian buf |> tag_of_int in
        let field = Endian.uint16 ~offset:(i * 20 + 2) endian buf |> field_of_int in
