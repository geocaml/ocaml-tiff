...
let decode input output_buffer =
  ...
  let input_bigstring : De.bigstring = Bigarray.Array1.create Bigarray.char Bigarray.c_layout input_len in
  let output_bigstring : De.bigstring = Bigarray.Array1.create Bigarray.char Bigarray.c_layout output_len in
  ...
  match De.Higher.uncompress ~refill ~flush ~w:window input_bigstring output_bigstring with
  | Ok () -> ()
  | Error (`Msg err) -> 
