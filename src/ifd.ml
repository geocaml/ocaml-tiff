...
let w_ifd width height compression planar_configuration
    photometric_interpretation sample_format bps_list data_bytecounts
    rows_per_strip document_name header samples_per_pixel w =
  let compression_int = compression_to_int compression in
  let planar_int = planar_configuration_to_int planar_configuration in
  let photometric_int =
    photometric_interpretation_to_int photometric_interpretation
  in
  ...
  let sample_format_int = sample_format_to_int sample_format in
  let make_entry = make_entry header.byte_order in
  let image_width, file_offset =
    make_entry file_offset ImageWidth (Ints [ width ])
  in
  let image_height, file_offset =
    make_entry file_offset ImageLength (Ints [ height ])
  in
  let bits_per_sample, file_offset =
    make_entry file_offset BitsPerSample (Ints bps_list)
  in
  let compression, file_offset =
    make_entry file_offset Compression (Ints [ compression_int ])
  in
  let photometric_interpretation, file_offset =
    make_entry file_offset PhotometricInterpretation (Ints [ photometric_int ])
  in
  let planar_config, file_offset =
    make_entry file_offset PlanarConfiguration (Ints [ planar_int ])
  in
  let rows_per_strip, file_offset =
    make_entry file_offset RowsPerStrip (Ints [ rows_per_strip ])
  in
  (*single strip*)
  let strip_bytecounts, file_offset =
    make_entry file_offset StripByteCounts (Ints data_bytecounts)
  in
  let samples_per_pixel, file_offset =
    make_entry file_offset SamplesPerPixel (Ints [ samples_per_pixel ])
  in
  let sample_format, file_offset =
    make_entry file_offset SampleFormat (Ints [ sample_format_int ])
  in

  let document_name, file_offset =
    make_entry file_offset DocumentName (String document_name)
  in

  let data_offsets = [ file_offset ] in

  let strip_offset, _ =
    make_entry file_offset StripOffsets (Ints data_offsets)
  in
  ...
  let entries =
    write_raw_ifd ~file_offset:header.offset header w make_entries
  in
  let ro = fun ~file_offset:_ _ -> () in
  let ifd = v_of_entries entries data_offsets data_bytecounts header ro in
  ifd
